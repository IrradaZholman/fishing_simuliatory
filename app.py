...
def api_register():
    try:
        ...
        if not fullname or not username or not email or not password:
            return jsonify({
                "success": False,
                "message": "Барлық жолдарды толтырыңыз"
            })
        ...
        return jsonify({
            "success": False,
            "message": "Бұл логин немесе email бұрын тіркелген"
        })
...
def api_login():
    try:
        ...
        if not username or not password:
            return jsonify({
                "success": False,
                "message": "Логин мен құпия сөзді енгізіңіз"
            })
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=10000)from flask import Flask, render_template, request, jsonify
import os
import psycopg2
import psycopg2.extras
from werkzeug.security import generate_password_hash, check_password_hash
